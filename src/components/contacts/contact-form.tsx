...
export function ContactForm({
  ...
  const [assignedSalesMemberId, setAssignedSalesMemberId] = useState<string | null>(null);
  const [contactType, setContactType] = useState<string>('corporate_lead');
  ...
  useEffect(() => {
    if (open) {
      ...
      setAssignedSalesMemberId(contact?.assigned_sales_member_id ?? null);
      setContactType(contact?.contact_type ?? 'corporate_lead');
      ...
      fetch('/api/fortline/sales-members')
        ...
        .then((d) => {
          if (d?.salesMembers) setSalesMembers(d.salesMembers);
        })
        .catch((e) => console.error('Failed to load sales reps:', e));
    }
  }, [open, contact]);
      ...
      if (isEdit && contactId) {
        const { error } = await supabase
          ...
          .update({
            ...
            assigned_sales_member_id: assignedSalesMemberId || null,
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="bg-popover border-border text-popover-foreground w-[95vw] sm:max-w-md max-h-[90vh] overflow-y-auto">
        <DialogHeader>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-2">
            <Label htmlFor="cf-sales-member" className="text-muted-foreground">
              Assigned Sales Rep (30 Reps)
            </Label>
            <select
              id="cf-sales-member"
              value={assignedSalesMemberId || ''}
              onChange={(e) => setAssignedSalesMemberId(e.target.value || null)}
              className="w-full h-9 rounded-md border border-border bg-muted px-3 text-sm text-foreground focus:outline-none focus:ring-1 focus:ring-primary cursor-pointer"
            >
              <option value="">Unassigned</option>
              {salesMembers.map((rep) => (
                <option key={rep.id} value={rep.id}>
                  {rep.name} ({rep.division} - {rep.member_code})
                </option>
              ))}
            </select>
          </div>
          ...
          <div className="space-y-2">
            ...
            <select
              ...
              className="w-full h-9 rounded-md border border-border bg-muted px-3 text-sm text-foreground focus:outline-none focus:ring-1 focus:ring-primary cursor-pointer"
            >
              <option value="corporate_lead">Corporate Lead (General)</option>
              <option value="enterprise_client">Enterprise Infrastructure Client</option>
              <option value="smb_lead">SMB Fleet & Laptop Buyer</option>
              <option value="datacenter_partner">Data Center Partner</option>
