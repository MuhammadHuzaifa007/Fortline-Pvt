...
export async function sendMessageToConversation(
  ...
  if (!conversation.first_response_at && conversation.created_at) {
    const diffSec = Math.round(
      (new Date().getTime() - new Date(conversation.created_at).getTime()) / 1000
    );
    convUpdate.first_response_at = nowIso;
    convUpdate.first_response_time_seconds = diffSec > 0 ? diffSec : 0;
  }
  ...
  // Update sales member activity timestamp if assigned
  if (conversation.assigned_sales_member_id) {
    try {
      await db
        ...
        .eq('id', conversation.assigned_sales_member_id);
    } catch {
      // Non-fatal
    }
  }
