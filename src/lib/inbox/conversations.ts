 ...
 */
/**
 * Base conversation select without foreign key embeds to sales members.
 * Safe against schema cache misses or unmigrated environments.
 */
export const CONVERSATION_BASE_SELECT =
  "*, contact:contacts(*, contact_tags(tags(*)))";

/**
 * Conversation select that embeds the contact plus its tags, and embeds
 * assigned_sales_member using explicit foreign key constraint.
 */
export const CONVERSATION_SELECT =
  "*, contact:contacts(*, contact_tags(tags(*))), assigned_sales_member:fortline_sales_members(*)";
...
export function normalizeConversation(raw: RawConversation): Conversation {
  ...
  const lastMessageText =
    raw.last_message_text || (raw as any).last_message_preview || "";
