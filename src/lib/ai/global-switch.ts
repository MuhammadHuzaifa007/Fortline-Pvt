 ...
 * 2. `itechskill_operations_config` (n8n operations config)
 * 3. `crm_ai_agent_audit_logs` (audit history)
 */
export async function updateGlobalAiAgentSettings(
  ...
  // 3. Upsert n8n operations config
  try {
    await db.from('itechskill_operations_config').upsert(
      {
        config_key: 'ai_agent_enabled',
    ...
    console.error(
      '[global-switch] non-fatal: failed to sync itechskill_operations_config:',
      n8nError
    )
  }
