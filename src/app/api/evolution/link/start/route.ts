...
export async function POST(request: Request) {
  try {
    ...
    const rawMode =
      typeof body.mode === 'string' ? body.mode.trim().toLowerCase() : 'qr';
    const mode: 'qr' | 'pairing' = rawMode === 'pairing' ? 'pairing' : 'qr';
    ...
    if (smErr) {
      console.error('[EVOLUTION LINK START] Error fetching sales member:', smErr);
      return NextResponse.json(
    ...
    const { data: channel, error: chErr } = await admin
      ...
      .select('id, sales_member_id, gateway_instance_id, channel_type, connection_status, pairing_state')
      .eq('sales_member_id', salesMemberId)
      .eq('account_id', accountId)
      .maybeSingle();

    if (chErr) {
      console.error('[EVOLUTION LINK START] Error fetching channel:', chErr);
      return NextResponse.json(
    ...
    const instanceName =
      channel.gateway_instance_id && channel.gateway_instance_id.trim()
        ? channel.gateway_instance_id.trim()
        : `fortline_rep_${channel.id.replace(/[^a-zA-Z0-9]/g, '_')}`;

    // 5. Ensure instance exists in Evolution (safe if already exists)
    const createRes = await createEvolutionInstance(instanceName);
    if (!createRes.success && !createRes.alreadyExists) {
      return NextResponse.json(
        { error: createRes.error || 'Failed to create Evolution instance' },
        { status: 502 },
      );
    }

    // 6. Automatically register webhook
    const webhookRes = await setEvolutionWebhook(instanceName);
    ...
    if (mode === 'pairing') {
      ...
      phoneParam = rawPhone.replace(/\D/g, '');
      if (!phoneParam) {
        return NextResponse.json(
    ...
    const connectRes = await getEvolutionConnection(instanceName, phoneParam);
    if (!connectRes.success) {
      return NextResponse.json(
        { error: connectRes.error || 'Failed to initiate WhatsApp connection' },
        { status: 502 },
      );
    }

    const connectData = (connectRes.data || {}) as Record<string, unknown>;

    // 8. Update fortline_channels with connecting state
    let updatePayload: Record<string, unknown> = {
      gateway_instance_id: instanceName,
      connection_status: 'connecting',
      pairing_state: mode === 'qr' ? 'qrcode' : 'pairing',
    ...
    if (updateErr && updateErr.code === '23514') {
      updatePayload = {
        gateway_instance_id: instanceName,
        connection_status: 'disconnected',
        pairing_state: mode === 'qr' ? 'qrcode' : 'connecting',
        webhook_status: 'active',
        updated_at: new Date().toISOString(),
      };
      ...
      if (fallback.error) {
        console.error('[EVOLUTION LINK START] Fallback channel update error:', fallback.error);
      }
    } else if (updateErr) {
      console.error('[EVOLUTION LINK START] Error updating channel state:', updateErr);
    }

    // 9. Extract pairing code if in pairing mode
    const pairingCode =
      mode === 'pairing'
        ? typeof connectData.pairingCode === 'string'
          ? connectData.pairingCode
          : typeof connectData.code === 'string'
          ? connectData.code
          : null
        : null;

    // 10. Return only safe data to frontend (never EVOLUTION_API_KEY)
    return NextResponse.json(
      {
        ok: true,
