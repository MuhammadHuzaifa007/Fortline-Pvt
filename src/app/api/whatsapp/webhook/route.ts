...
export async function POST(request: Request) {
  ...
  // Direct CRM event processing without external intermediary webhooks.
  after(async () => {
    try {
...
async function processWebhook(body: { entry?: WhatsAppWebhookEntry[] }) {
  ...
  for (const entry of body.entry) {
    for (const change of entry.changes) {
      // Template-lifecycle events (status / quality / components updates from Meta)
      ...
      // Channel routing: resolve channel mapping
      // phone_number_id -> sales member -> contact -> conversation -> message
      ...
      const salesMemberId = channelRow?.sales_member_id || config?.sales_member_id || null
      const accountId = channelRow?.account_id || config?.account_id || null
      const configOwnerUserId = config?.user_id || '00000000-0000-0000-0000-000000000000'
      const decryptedAccessToken = config?.access_token ? decrypt(config.access_token) : ''

      if (!channelRow && !config) {
        console.warn('[webhook] unmapped channel event for phone_number_id:', phoneNumberId)
        // Record notification for invalid/unmapped channel event
        await supabaseAdmin().from('notifications').insert({
          account_id: accountId || '00000000-0000-0000-0000-000000000000',
          user_id: configOwnerUserId,
          type: 'channel_error',
          title: `Unmapped WhatsApp Channel Event: ${phoneNumberId}`,
          body: `Received inbound WhatsApp event for unmapped phone number ID ${phoneNumberId}.`,
          severity: 'warning',
...
async function processMessage(
  ...
  if (salesMemberId && !conversation.assigned_sales_member_id) {
    convUpdates.assigned_sales_member_id = salesMemberId
  }
  if (phoneNumberId && !conversation.channel_phone_number_id) {
    convUpdates.channel_phone_number_id = phoneNumberId
  }
  ...
  // Touch sales member inbound activity and presence
  if (salesMemberId) {
    await supabaseAdmin()
  ...
  if (!contactRecord.is_spam && !flowConsumed) {
  ...
  // inserted, then the steps never run.
  ...
  // Fortline CRM operates with direct WhatsApp Cloud API for executive oversight
  // and does not dispatch autonomous replies without human agent involvement.
