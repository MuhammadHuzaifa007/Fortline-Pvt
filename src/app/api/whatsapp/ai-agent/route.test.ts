...
describe('GET /api/whatsapp/ai-agent', () => {
...
describe('PATCH /api/whatsapp/ai-agent', () => {
  it('rejects unauthenticated requests with 401', async () => {
    mocks.getCurrentAccount.mockRejectedValue({ message: 'Unauthorized', status: 401 });

    const req = new Request('http://localhost/api/whatsapp/ai-agent', {
      method: 'PATCH',
  ...
  it('rejects invalid or non-boolean body with 400', async () => {
    const req1 = new Request('http://localhost/api/whatsapp/ai-agent', {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
    ...
    const req2 = new Request('http://localhost/api/whatsapp/ai-agent', {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({}),
    });
