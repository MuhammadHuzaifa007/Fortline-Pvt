...
export default function ContactsPage() {
  ...
  const fetchContacts = useCallback(async () => {
    ...
    if (selectedTagIds.length > 0) {
      ...
      let query = supabase
        .from('contacts')
        .select('*, assigned_sales_member:fortline_sales_members(*)', { count: 'exact' })
      ...
      let { data, count: exactCount, error } = await query;
      ...
      if (
        ...
        if (term) {
          const like = `%${term}%`;
          fallbackQuery = fallbackQuery.or(`name.ilike.${like},phone.ilike.${like},email.ilike.${like}`);
        }
        const fallbackRes = await fallbackQuery;
        data = fallbackRes.data;
        exactCount = fallbackRes.count;
        ...
        <div className="overflow-x-auto">
          ...
          <TableHeader>
            <TableRow className="border-border hover:bg-transparent">
              ...
              <TableHead className="text-muted-foreground hidden lg:table-cell">Assigned Rep</TableHead>
              <TableHead className="text-muted-foreground hidden xl:table-cell">Category</TableHead>
              ...
              contacts.map((contact) => (
                ...
                >
                  ...
                  <TableCell className="hidden lg:table-cell">
                    {(contact as any).assigned_sales_member ? (
                      <div className="flex flex-col">
                        <span className="font-semibold text-foreground text-xs">
                          {(contact as any).assigned_sales_member.name}
                        </span>
                        <span className="text-[10px] text-muted-foreground">
                          {(contact as any).assigned_sales_member.division}
                        </span>
                      </div>
                    ) : (
                      <span className="inline-flex items-center rounded bg-amber-500/10 px-2 py-0.5 text-[10px] font-medium text-amber-500 border border-amber-500/20">
                        Unassigned
                      </span>
                    )}
                  </TableCell>
                  <TableCell className="hidden xl:table-cell text-xs text-muted-foreground">
                    <span className="capitalize">
                      {contact.contact_type ? contact.contact_type.replace(/_/g, ' ') : 'Corporate Lead'}
                    </span>
                  </TableCell>
