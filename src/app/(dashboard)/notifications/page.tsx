...
const TYPE_ICON: Record<string, typeof Bell> = {
  ...
  new_unassigned_lead: UserPlus,
  unanswered_conversation: Flame,
  sla_breach: AlertOctagon,
  overdue_conversation: Clock,
  channel_disconnected: Radio,
  webhook_failure: AlertTriangle,
  delivery_failure: AlertTriangle,
  channel_error: Radio,
  system_exception: ShieldAlert,
  ceo_confirmation: ShieldAlert,
};
      ...
      {notifications.length === 0 ? (
        <div className="flex h-48 flex-col items-center justify-center rounded-xl border border-dashed border-border bg-muted/40">
          ...
          <p className="mt-1 text-xs text-muted-foreground">
            Operational alerts and SLA warnings will appear here in real time.
          </p>
        </div>
      ) : (
        <ul className="space-y-2">
          {notifications.map((n) => {
            ...
            const severity = (n as any).severity || (n.type.includes('breach') || n.type.includes('disconnect') ? 'critical' : 'warning');
                ...
                >
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2">
                      ...
                      {severity === 'critical' && (
                        <span className="text-[9px] uppercase px-1.5 py-0.5 rounded bg-rose-500/20 text-rose-500 font-bold">
                          Critical
                        </span>
                      )}
                      {isUnread && (
